...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen">
      {/* --- 사이드바 --- */}
      <aside className={`${isCollapsed ? "w-20" : "w-64"} bg-slate-950 text-white transition-all duration-300 flex flex-col sticky top-0 h-screen`}>
        <div className="p-6 flex justify-between items-center border-b border-slate-900">
          {!isCollapsed && <h2 className="text-xl font-black italic tracking-tighter">SUNFLEX</h2>}
        ...
        <nav className="flex-1 p-4 space-y-2 uppercase font-bold text-[11px] tracking-widest">
          <a href="/inventory" className="flex items-center gap-4 p-3 bg-blue-600 rounded-xl text-white shadow-lg shadow-blue-900/20">
             <span>📦</span> {!isCollapsed && "Inventory"}
          </a>
          <div className="opacity-20 flex items-center gap-4 p-3 cursor-not-allowed">
        ...
        <div className="p-4 border-t border-slate-900">
          <div className={`flex items-center ${isCollapsed ? "justify-center" : "justify-between"} bg-slate-900 p-2 rounded-2xl`}>
            ...
            <button 
              ...
              className={`w-10 h-5 rounded-full relative transition-all ${isCEO ? "bg-emerald-500" : "bg-slate-700"}`}
            >
