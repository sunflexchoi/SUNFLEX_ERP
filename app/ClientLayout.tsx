    ...
    <div className="flex min-h-screen font-sans" style={{ fontFamily: "'Geist Sans', sans-serif" }}>
      ...
      <aside className={`${isCollapsed ? "w-20" : "w-64"} bg-[#0a0a0b] text-white transition-all duration-300 flex flex-col sticky top-0 h-screen z-50`}>
        ...
        <div className="p-4 border-t border-white/5">
          <div className={`flex items-center ${isCollapsed ? "justify-center" : "justify-between"} bg-white/5 p-2 rounded-2xl`}>
            {!isCollapsed && <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{isCEO ? "CEO" : "Staff"}</span>}
            <button onClick={() => setIsCEO(!isCEO)} className={`w-10 h-5 rounded-full relative transition-all ${isCEO ? "bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.4)]" : "bg-slate-700"}`}>
      ...
      <main className="flex-1 bg-[#fcfcfd] overflow-x-hidden">
        {children}
      </main>
    </div>
  );
