  ...
  <div className="space-y-3 font-sans">
    {data.map((item) => (
      <div key={item.id} className="flex items-center gap-6 bg-white p-3 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="flex-1 grid grid-cols-4 items-center gap-4 text-sm">
          <div className="font-bold text-blue-900 leading-none">{item.serial_no}</div>
          <div className="font-bold text-slate-800 leading-none">{item.name_ko}</div>
          <div className="text-right font-black text-emerald-600 leading-none">{item.stock_qty}pcs</div>
          <div className="text-right font-black text-slate-900 leading-none tracking-tighter font-mono">₩{item.price?.toLocaleString()}</div>
        </div>
      </div>
    ))}
  </div>
  ...
  <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm font-sans text-xs">
    <table className="w-full text-left">
      <thead className="bg-slate-50 border-b border-slate-100 font-bold text-slate-400 uppercase tracking-widest text-[10px]">
        <tr><th className="p-4">Serial</th><th className="p-4">Product</th><th className="p-4 text-right">Qty</th><th className="p-4 text-right">Price</th></tr>
      </thead>
